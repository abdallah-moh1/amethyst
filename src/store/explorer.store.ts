...
export const useExplorerStore = create<ExplorerStore>((set) => ({
    ...
    removeNoteFromTree: (noteId) => set((state) => {
        const removeFromNodes = (nodes: TreeNode[]): TreeNode[] =>
            nodes
                ...
                .map((n) =>
                    n.type === 'notebook'
                        ? { ...n, children: removeFromNodes(n.children) }
                        : n,
                );
        return { tree: removeFromNodes(state.tree) };
    }),
    updateNoteInTree: (updatedNote) => set((state) => {
    ...
    addNotebookToTree: (notebook) => set((state) => {
        ...
        if (isRoot) {
            if (state.tree.some(
                (n) => n.type === 'notebook' && normalize(n.path) === cleanPath
            )) {
                return state;
            }
    ...
    removeNotebookFromTree: (targetPath) => set((state) => {
        const normalize = (p: string) => p.replace(/\/+$/, '');
            ...
            nodes
                .filter((node) => {
                    ...
                    return !(
                        ...
                        nodePath.startsWith(cleanTarget + '/')
                    );
                })
                .map((node) => {
                    if (node.type === 'notebook') {
    ...
    updateNotebookInTree: (oldPath, updatedNotebook) => set((state) => {
        ...
        const rewritePaths = (nodes: TreeNode[]): TreeNode[] =>
            nodes.map((node) => {
                ...
                const newNodePath = node.path === oldPath
                    ? newPath
                ...
                return {
                    ...node,
                    name: node.path === oldPath ? updatedNotebook.path.split('/').at(-1)! : node.name,
                    path: newNodePath,
